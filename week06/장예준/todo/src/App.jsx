...
// useState는 컴포넌트에 state 변수를 수정 값을 저장 // 코드에서 사용되는 useState는 input과 todos 두개의 state
...
function App() {
  ...
  // 위코드가 필요한 이유 localStorage에 저장하고 페이지를 새로 고침하더라고 이전에 저장된 내용이 남아있게 하기 위해서
  // chatgpt 제공 코드
  ...
  // 공부한점 == react는 자동으로 화면을 업데이트 해주지만 외부저장소 서버 콘솔 타이머는(side effect) 직접 동기화 해주어야함 
...
// 정리
//useEffect는 todos가 바뀔 때마다 실행되어야 하기에 useEffect를 사용
...
// 추가, 수정, 삭제된 todos를 문자열로 변환해 브라우저 저장소(localStorage)에 저장하는 역할을 합니다.
  const handleSubmit = (e) => { // e == event 객체 폼을 제출 --> 자동으로 객체를 함수에 넘김김 
    ...
    setTodos([...todos, newTodo]); // ...todo 복사
    //
